...
const Navbar = () => {
  ...
  return (
    <nav aria-label="Main navigation" className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 px-6 py-4 flex justify-between items-center ${navBgClass}`}>
      <Link to="/" aria-label="Travco Holidays - Home" className="flex items-center gap-2">
        <img src="https://logo.clearbit.com/travcoholidays.com" onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.style.display='none'; e.currentTarget.nextSibling.style.display='block'; }} alt="Travco Holidays Logo" className="h-8 md:h-12 object-contain" /><span className="text-2xl font-serif font-bold tracking-tighter uppercase transition-colors" style={{display: 'none'}}>Travco Holidays</span>
      </Link>
